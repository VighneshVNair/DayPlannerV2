      ...
      {tasks.map((task, index) => {
        ...
        return (
          <div 
            ...
            className={`
              group relative flex items-center p-3 rounded-r-xl transition-all duration-200 
              ${containerClasses}
          ...
          >
            ...
            <div className="flex flex-col items-center mr-4 min-w-[60px]">
              <span className={`text-xs font-mono ${isActive ? 'text-white font-bold' : 'text-slate-400'}`}>
              ...
              <div className="mb-1">
                {isEditing ? (
                  <div className="space-y-2">
                      ...
                      <div className="flex space-x-2" onMouseDown={(e) => e.preventDefault() /* prevent blur */}>
                          {COLORS.map(c => (
                              <button
                                  key={c}
                                  onClick={(e) => {
                                  ...
                                  className={`w-5 h-5 rounded-full transition-transform hover:scale-110 ${task.color === c ? 'ring-2 ring-white scale-110' : 'opacity-70 hover:opacity-100'}`}
              ...
              {!isEditing && (
                <div className="flex items-center text-xs text-slate-400 space-x-4">
                    ...
                    <div className="flex items-center" title="Completed / Expected Pomodoros">
                        <svg className="w-3 h-3 mr-1 opacity-70" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>
            ...
            {!isCompleted && !isEditing && (
                <div className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-black/40 backdrop-blur-sm shadow-sm border border-white/10 rounded-lg">
                    <button 
                        onClick={(e) => { e.stopPropagation(); onDeleteTask(task.id); }}
                        className="p-2 text-slate-400 hover:text-red-400 rounded-lg hover:bg-white/10"
                    >
